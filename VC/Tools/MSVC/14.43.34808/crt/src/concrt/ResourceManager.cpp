...
namespace details
{
    ...
    void ResourceManager::DetermineTopology()
    {
        ...
        m_pGlobalNumaNodes = _concrt_new GlobalNumaNode_v2[m_numaNodeCount];
...
#pragma warning(pop)
                    ...
                    case RelationNumaNode:
                        {
                            ...
                            m_pGlobalNumaNodes[numaIndex].Initialize(pSysInfoEx->NumaNode.NodeNumber,
                                                                     pSysInfoEx->NumaNode.GroupCount,
                                                                     pSysInfoEx->NumaNode.GroupMasks);
                            ++numaIndex;
                        }
                        break;
                    default:
        ...
        if (s_countProcessorPackages)
        {
            ...
            {
                auto& globalNode = m_pGlobalNodes[i];
                bool fDiscovered = false;
                for (unsigned int j = 0; j < m_numaNodeCount && !fDiscovered; ++j)
                {
                    const auto& globalNumaNode = m_pGlobalNumaNodes[j];
                    for (WORD k = 0; k < globalNumaNode.m_groupCount && !fDiscovered; ++k)
                    {
                        const auto& groupMask = globalNumaNode.m_groupMasks[k];
                        if (globalNode.m_processorGroup == groupMask.Group &&
                            (globalNode.m_nodeAffinity & groupMask.Mask))
                        {
                            // The node should be fully contained within the NUMA node.
                            ASSERT((globalNode.m_nodeAffinity & groupMask.Mask) == globalNode.m_nodeAffinity);
                            globalNode.m_numaNodeNumber = globalNumaNode.m_numaNodeNumber;
                            fDiscovered = true;
                        }
                    }
                }
