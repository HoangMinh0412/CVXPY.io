...
namespace details
{
    ...
    /// </summary>
    struct GlobalNumaNode_v2
    {
        // The NUMA node number for this NUMA node
        DWORD m_numaNodeNumber{0};

        // The number of processor groups described by m_groupMasks.
        WORD m_groupCount{0};

        // An array of group numbers and affinity masks.
        GROUP_AFFINITY *m_groupMasks{nullptr};

        GlobalNumaNode_v2() = default;
        GlobalNumaNode_v2(const GlobalNumaNode_v2&) = delete;
        GlobalNumaNode_v2& operator=(const GlobalNumaNode_v2&) = delete;

        /// <summary>
        ///     Initializes a numa node.
        /// </summary>
        void Initialize(const DWORD numaNodeNumber, const WORD rawGroupCount, const GROUP_AFFINITY *const groupMasks)
        {
            ...
            //   This field was introduced in TBD Release Iron.
            //   On earlier versions, this value is always 0.
            const WORD totalGroupCount = rawGroupCount == 0 ? 1 : rawGroupCount;

            // Filter out all-zero affinity masks.
            for (auto src = groupMasks; src != groupMasks + totalGroupCount; ++src) {
                if (src->Mask != 0) {
                    ++m_groupCount;
                }
            }

            // m_groupCount is now the number of non-zero affinity masks.
            if (m_groupCount != 0) {
                m_groupMasks = _concrt_new GROUP_AFFINITY[m_groupCount];

                auto dst = m_groupMasks;

                for (auto src = groupMasks; src != groupMasks + totalGroupCount; ++src) {
                    if (src->Mask != 0) {
                        memcpy(dst, src, sizeof(GROUP_AFFINITY));
                        ++dst;
                    }
                }
            }
